...
Run as a sub-agent to avoid consuming the main context:
...
For companies using Greenhouse, the JSON API (`boards-api.greenhouse.io/v1/boards/{slug}/jobs`) returns clean structured data. Use as a fast complement to Level 1 — it is faster than Playwright but only works with Greenhouse.
...
5. **Level 2 — Greenhouse APIs** (parallel):
   ...
   c. Accumulate into candidate list (dedup with Level 1)
